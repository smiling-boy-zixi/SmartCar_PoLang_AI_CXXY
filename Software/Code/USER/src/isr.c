...
void PIT_IRQHandler(void)
{
    ...
    if(PIT_FLAG_GET(PIT_CH2))  //编码器
    {
				encoder_L = -qtimer_quad_get(QTIMER_1,QTIMER1_TIMER0_C0 );  //由于两个编码器镜像安装，所以两个编码器读出来的值是一正一负的
				encoder_R = qtimer_quad_get(QTIMER_1,QTIMER1_TIMER2_C2 );
				speed_ctrl(speed_set,1,0);				
